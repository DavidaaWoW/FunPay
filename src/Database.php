...
class Database implements DatabaseInterface {
	...
	private function castType($arg, $spec_type, $slashes = false, $apostrophe = false) {
		...
		if( $slashes && gettype($arg) == 'string' ) {
			$arg = "\'$arg\'";
		}
		if( $slashes && gettype($arg) == 'NULL' ) {
			$arg = 'NULL';
		}
	...
	public function buildQuery(string $query, array $args = []): string {
		...
		foreach( $matches[0] as $i => $match ) {
			...
			if( in_array($match, $this->_spec_types) ) {
				$spec_type = array_search($match, $this->_spec_types);
